<script lang="ts">
	...
	import CompanyCreateDialog from './CompanyCreateDialog.svelte';
	...
	async function deleteCompany(uid: string) {
		companyDeleting[uid] = true;
		const result = await companiesApi.deleteCompany(uid);
		if (result.ok) {
			updateCompanies();
		} else {
			snackbar.show(result.error.message);
		}
		companyDeleting[uid] = false;
	}
